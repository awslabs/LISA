...
export const useSessionEncryption = () => {
   ...
   */
    const getEncryptionConfig = useCallback(async (): Promise<EncryptionConfig> => {
        try {
   ...
   */
    const generateDataKey = useCallback(async (userId: string, sessionId: string): Promise<{ plaintext: string; encrypted: string }> => {
        try {
   ...
   */
    ...
    }, [encryptionConfig.enabled, generateDataKey]);
