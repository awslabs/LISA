...
export class LisaApiDeploymentConstruct extends Construct {
    constructor (scope: Stack, id: string, props: LisaApiDeploymentProps) {
        ...
        const api_url = config.restApiConfig.domainName ? `https://${config.apiGatewayConfig?.domainName}` : `https://${restApiId}.execute-api.${Aws.REGION}.${Aws.URL_SUFFIX}/${config.deploymentStage}`;
