...
from lisa.domain.domain_objects import BedrockAgentActionTool, BedrockAgentAliasSummary, BedrockAgentDiscoveryItem
...
def _pick_suggested_alias(aliases: list[BedrockAgentAliasSummary]) -> str | None:
    ...
    for a in aliases:
        if a.agentAliasStatus == "PREPARED":
            return str(a.agentAliasId)
    return str(aliases[0].agentAliasId) if aliases else None
