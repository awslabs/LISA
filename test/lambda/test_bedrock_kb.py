...
class TestRetrieveDocuments:
    """Test document retrieval from Bedrock Knowledge Base."""
    ...
    def test_retrieve_documents_success(self, mock_bedrock_runtime_client, sample_repository):
        """Test successful document retrieval."""
        # Arrange
        mock_bedrock_runtime_client.retrieve.return_value = {
            "retrievalResults": [
                {
                ...
                },
                {
                    ...
                    "location": {"s3Location": {"uri": "s3://bucket/folder/doc2.txt"}},
                },
            ]
        }
        ...
        # Assert
        assert len(result) == 2
        assert result[0]["page_content"] == "Document content 1"
        assert result[0]["metadata"]["source"] == "s3://bucket/doc1.pdf"
        assert result[0]["metadata"]["name"] == "doc1.pdf"
        assert result[0]["metadata"]["repository_id"] == "test-repo"

        assert result[1]["page_content"] == "Document content 2"
        assert result[1]["metadata"]["name"] == "doc2.txt"

        # Verify API call
        mock_bedrock_runtime_client.retrieve.assert_called_once_with(
            knowledgeBaseId="KB123456",
            retrievalQuery={"text": "test query"},
            retrievalConfiguration={"vectorSearchConfiguration": {"numberOfResults": 5}},
        )

    def test_retrieve_documents_empty_results(self, mock_bedrock_runtime_client, sample_repository):
        """Test retrieval with no results."""
        # Arrange
        mock_bedrock_runtime_client.retrieve.return_value = {"retrievalResults": []}
    ...
    def test_retrieve_documents_missing_location(self, mock_bedrock_runtime_client, sample_repository):
        """Test retrieval with missing location data."""
        # Arrange
        mock_bedrock_runtime_client.retrieve.return_value = {
            "retrievalResults": [
                {
        ...
        assert result[0]["metadata"]["name"] is None
