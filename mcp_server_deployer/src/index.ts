...
export const handler = async (event: any) => {
    ...
    if (!event.mcpServerConfig) {
        console.log(`mcpServerConfig not provided in ${JSON.stringify(event)}`);
        throw new Error('mcpServerConfig not provided');
    }
    const mcpServerConfig = event.mcpServerConfig;
    process.env['LISA_MCP_SERVER_CONFIG'] = JSON.stringify(mcpServerConfig);
