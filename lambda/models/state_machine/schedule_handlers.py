...
def handle_schedule_creation(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    if not auto_scaling_group:
        ...
        logger.warning(f"Model {model_id} created successfully but scheduling failed. User can add schedule later via API.")
        return output_dict
    ...
    try:
        ...
        if "Item" in response:
            ...
            if current_scheduling_config != new_scheduling_config:
                ...
                existing_arns = current_scheduling_config.get("scheduledActionArns", []) if current_scheduling_config else []
