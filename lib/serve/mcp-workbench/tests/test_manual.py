...
def check_api_endpoints(base_url: str):
    ...
    try:
        ...
        response = requests.post(
            f"{base_url}/mcp/tools/echo/call",
            json={"arguments": {"message": "Hello, MCP Workbench!"}}
        )
        if response.status_code == 200:
        ...
        response = requests.post(
            f"{base_url}/mcp/tools/add_numbers/call",
        ...
        response = requests.post(
            f"{base_url}/mcp/tools/greeting/call",
            json={"arguments": {"name": "Alice", "style": "enthusiastic"}}
        )
        if response.status_code == 200:
            result = response.json()
            print(f"   ✓ Result: {result['result']}")
