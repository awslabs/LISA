...
from lisa.utilities.chunking_strategy_factory import ChunkingStrategyFactory
from lisa.utilities.constants import DOCX_FILE, PDF_FILE, RAG_FILES
...
def _extract_text_by_content_type(content_type: str, s3_object: dict[str, Any]) -> str:
...
def _extract_text_content(s3_object: dict[str, Any]) -> str:
    """
    ...
    if body is None:
        raise RagUploadException("S3 object body is missing")
    raw = body.read()
    if isinstance(raw, str):
        return raw
    return cast(bytes, raw).decode("utf-8", errors="replace")
...
def generate_chunks(ingestion_job: IngestionJob) -> list[Document]:
    ...
    doc_chunks = cast(list[Document], ChunkingStrategyFactory.chunk_documents(docs, chunk_strategy))
