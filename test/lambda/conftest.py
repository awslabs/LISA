...
def mock_auth_context():
    ...
    with patch("utilities.auth.get_groups") as mock_get_groups, patch(
        "utilities.auth.is_admin"
    ) as mock_is_admin, patch("utilities.auth.get_username") as mock_get_username, patch(
        "utilities.auth.get_user_context"
...
def mock_repositories():
    ...
    with patch("repository.lambda_functions.vs_repo") as mock_vs_repo, patch(
        ...
        yield {"vs_repo": mock_vs_repo, "job_repo": mock_job_repo}
...
def mock_repository_services():
    ...
    yield {"vs_repo": mock_vs_repo, "doc_repo": mock_doc_repo, "ingestion_service": mock_ingestion_service}
...
def aws_moto_services():
    ...
    with mock_aws():
        ...
        ssm.put_parameter(Name="test-api-url", Value="https://api.test.com", Type="String")
        ssm.put_parameter(Name="test-secret-name", Value="test-secret-arn", Type="String")
        ssm.put_parameter(
        ...
        secretsmanager.create_secret(Name="test-secret", SecretString=json.dumps({"password": "test-password"}))
...
def mock_session_auth():
    ...
    with patch("utilities.auth.get_username", side_effect=get_username_from_event) as mock_get_username, patch(
        "utilities.common_functions.get_session_id", side_effect=get_session_id_from_event
    ) as mock_get_session_id:

        yield {"get_username": mock_get_username, "get_session_id": mock_get_session_id}
...
class RepositoryTestHelper:
    ...
    def create_user_event(repository_id: str, username: str = "test-user", queryStringParameters=None):
        ...
        return {
            "requestContext": {"authorizer": {"username": username, "groups": '["user-group"]'}},
            "pathParameters": {"repositoryId": repository_id},
            "queryStringParameters": queryStringParameters or {},
        }
    ...
    def create_repository_event(username: str, groups: list, path_params=None, query_params=None, body=None):
        ...
        event = {
            "requestContext": {"authorizer": {"claims": {"username": username}, "groups": json.dumps(groups)}},
...
class SessionTestHelper:
    ...
    def create_session_event(session_id: str = "test-session", username: str = "test-user", **kwargs):
        ...
        event = {
            "requestContext": {"authorizer": {"claims": {"username": username}}},
