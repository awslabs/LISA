...
def test_vector_store_opensearch_basic():
    ...
    with patch('utilities.vector_store.ssm_client') as mock_ssm_client, \
         patch('utilities.vector_store.session') as mock_session, \
         patch('utilities.vector_store.OpenSearchVectorSearch') as mock_opensearch:
        ...
        result = get_vector_store_client("test-repo", "test-index", mock_embeddings)
...
def test_vector_store_pgvector_basic():
    ...
    with patch('utilities.vector_store.ssm_client') as mock_ssm_client, \
         patch('utilities.vector_store.PGVector') as mock_pgvector, \
         patch('utilities.vector_store.get_lambda_role_name') as mock_get_role, \
         patch('utilities.vector_store.generate_auth_token') as mock_generate_token:
