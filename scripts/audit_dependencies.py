                    ...
                    for group_name, group_data in data["tool"]["poetry"].get("group", {}).items():
                        if "dependencies" in group_data:
                            for package, spec in group_data["dependencies"].items():
                                ...
                                self.package_versions[package][f"{rel_path} [tool.poetry.group.{group_name}]"].add(version_spec)
                ...
                if "project" in data and "optional-dependencies" in data["project"]:
                    for group_name, deps in data["project"]["optional-dependencies"].items():
                        for dep in deps:
                            ...
                            if parsed:
                                package, version_spec = parsed
                                self.package_versions[package][f"{rel_path} [project.optional-dependencies.{group_name}]"].add(version_spec)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Audit dependency versions across LISA components",
...
Examples:
  ...
  python scripts/audit_dependencies.py --verbose
        """
    )
    parser.add_argument(
        "--verbose", "-v",
        action="store_true",
        help="Show verbose output including all scanned files"
    )
