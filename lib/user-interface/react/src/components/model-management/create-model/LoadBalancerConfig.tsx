...
export function LoadBalancerConfig(props: FormProps<ILoadBalancerConfig>): ReactElement {
    return (
        <>
            <Container header={<Header variant="h2">Health Check Config</Header>}>
                <FormField label="Path" errorText={props.formErrors?.loadBalancerConfig?.healthCheckConfig?.path}>
                    <Input
                        value={props.item.healthCheckConfig.path}
                        inputMode="text"
                        onBlur={() => props.touchFields(['loadBalancerConfig.healthCheckConfig.path'])}
                        onChange={({ detail }) => {
                ...
                >
                    <Grid gridDefinition={[{ colspan: 10 }, { colspan: 2 }]} disableGutters={true}>
                        <Input
                            ...
                            type="number"
                            inputMode="numeric"
                            onBlur={() => props.touchFields(['loadBalancerConfig.healthCheckConfig.interval'])}
                            onChange={({ detail }) => {
                                props.setFields({
                                    'loadBalancerConfig.healthCheckConfig.interval': Number(detail.value)
                                });
                            }}
                        />
                ...
                <FormField label="Timeout" errorText={props.formErrors?.loadBalancerConfig?.healthCheckConfig?.timeout}>
                    <Grid gridDefinition={[{ colspan: 10 }, { colspan: 2 }]} disableGutters={true}>
                        <Input
                            ...
                            type="number"
                            inputMode="numeric"
                            onBlur={() => props.touchFields(['loadBalancerConfig.healthCheckConfig.timeout'])}
                            onChange={({ detail }) => {
                                props.setFields({
                                    'loadBalancerConfig.healthCheckConfig.timeout': Number(detail.value)
                                });
                            }}
                        />
                        <span style={{ lineHeight: '2.5em', paddingLeft: '0.5em' }}>seconds</span>
                ...
                >
                    <Input
                        value={props.item.healthCheckConfig.healthyThresholdCount.toString()}
                        type="number"
                        inputMode="numeric"
                        onBlur={() => props.touchFields(['loadBalancerConfig.healthCheckConfig.healthyThresholdCount'])}
                        onChange={({ detail }) => {
                            props.setFields({
                                'loadBalancerConfig.healthCheckConfig.healthyThresholdCount': Number(detail.value)
                            });
                        }}
                    />
                </FormField>
                ...
                >
                    <Input
                        value={props.item.healthCheckConfig.unhealthyThresholdCount.toString()}
                        type="number"
                        inputMode="numeric"
                        onBlur={() =>
                            props.touchFields(['loadBalancerConfig.healthCheckConfig.unhealthyThresholdCount'])
                        }
                        onChange={({ detail }) => {
                            props.setFields({
                                'loadBalancerConfig.healthCheckConfig.unhealthyThresholdCount': Number(detail.value)
                            });
                        }}
                    />
                </FormField>
