...
from lisa.utilities.constants import DEFAULT_PAGE_SIZE, DEFAULT_TIME_LIMIT_HOURS, MAX_PAGE_SIZE, MIN_PAGE_SIZE
...
class ListJobsParams:
    ...
    def _parse_page_size(query_params: dict[str, str]) -> int:
        ...
        return max(int(MIN_PAGE_SIZE), min(page_size, int(MAX_PAGE_SIZE)))
    ...
    def _parse_last_evaluated_key(query_params: dict[str, str]) -> dict[str, str] | None:
        ...
        try:
            ...
            if not isinstance(loaded, dict) or not all(
                ...
                raise ValidationError("Invalid lastEvaluatedKey format: expected a JSON object of string keys/values")
