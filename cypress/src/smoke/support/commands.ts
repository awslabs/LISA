...
Cypress.Commands.add('loginAs', (role = 'user') => {
    cy.session(
        ...
        () => {
            ...
            // Wait for login to complete and verify we're on the main app
            cy.url().should('not.include', '/login');
            cy.get('body').should('be.visible');
        },
        {
            validate: () => {
                // Validate that the session is still valid by visiting home page
                cy.visit('/');
                cy.get('body').should('be.visible');
                
                // For admin users, verify admin button exists
                if (role === 'admin') {
                    cy.get('button[aria-label="Administration"]', { timeout: 10000 }).should('exist');
                } else {
                    // For regular users, verify admin button does not exist
                    cy.get('button[aria-label="Administration"]').should('not.exist');
                }
            },
