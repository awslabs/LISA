...
def patch_dynamodb_methods():
    with patch("lisa.rag.rag_document_repo.boto3.resource") as mock_resource, patch(
        "lisa.rag.rag_document_repo.boto3.client"
    ) as mock_client:
...
def test_rag_document_repository_init():
    ...
    with patch("lisa.rag.rag_document_repo.boto3.resource") as mock_resource, patch(
        "lisa.rag.rag_document_repo.boto3.client"
    ) as mock_client, patch("lisa.rag.rag_document_repo.VectorStoreRepository") as mock_vs_repo_class:
...
def test_list_all_with_join_docs(sample_rag_document):
    ...
    with patch("lisa.rag.rag_document_repo.boto3.resource") as mock_resource, patch(
        "lisa.rag.rag_document_repo.boto3.client"
    ) as mock_client, patch("lisa.rag.rag_document_repo.VectorStoreRepository") as mock_vs_repo_class, patch(
        "lisa.rag.rag_document_repo.RagDocumentRepository._get_subdoc_ids", return_value=["subdoc1"]
    ):
