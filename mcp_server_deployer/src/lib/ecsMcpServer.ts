...
`;
    ...
    private createContainerDefinition (
        ...
        if (isPrebuiltImage && mcpServerConfig.serverType === 'stdio') {
            ...
            bashScript += 'eval exec /root/.local/bin/mcp-proxy --stateless --allow-origins=\'*\' --transport streamablehttp --port=8080 --host=0.0.0.0 "$START_CMD"; ';
            bashScript += 'elif [ -f /root/.cargo/bin/mcp-proxy ]; then ';
            bashScript += 'eval exec /root/.cargo/bin/mcp-proxy --stateless --allow-origins=\'*\' --transport streamablehttp --port=8080 --host=0.0.0.0 "$START_CMD"; ';
            bashScript += 'elif command -v mcp-proxy >/dev/null 2>&1; then ';
            bashScript += 'eval exec mcp-proxy --stateless --allow-origins=\'*\' --transport streamablehttp --port=8080 --host=0.0.0.0 "$START_CMD"; ';
            bashScript += 'else ';
