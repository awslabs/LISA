...
from lisa.rag.config.params import ListJobsParams
...
from lisa.utilities.auth import (
    admin_only,
    get_groups,
    get_user_context,
    get_username,
...
def similarity_search(event: dict, context: dict) -> dict[str, Any]:
    ...
    """
    query_string_params: dict[str, Any] = event.get("queryStringParameters") or {}
...
def delete_documents(event: dict, context: dict) -> dict[str, Any]:
    ...
    if document_ids:
        rag_documents = [
            doc
            for doc in (doc_repo.find_by_id(document_id=document_id) for document_id in document_ids)
            if doc is not None
        ]
