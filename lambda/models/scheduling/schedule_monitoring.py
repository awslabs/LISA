...
def handle_successful_scaling(model_id: str, auto_scaling_group: str, detail: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        update_model_status(model_id, new_status, f"Auto Scaling action completed successfully")
...
def retry_failed_scaling(event: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "statusCode": 200,
            "body": json.dumps({
                "message": "Retry count reset - normal scheduling will retry the operation",
                "modelId": model_id
            })
        }
...
def reset_retry_count(model_id: str) -> None:
    ...
    try:
        model_table.update_item(
            Key={"model_id": model_id},
            UpdateExpression="REMOVE autoScalingConfig.scheduling.lastScheduleFailure"
        )
