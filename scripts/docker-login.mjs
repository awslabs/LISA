...
/**
 * Docker ECR login - replaces make dockerLogin.
 */
...
function main() {
  ...
  for (const account of ecrAccounts) {
    console.log(`Logging into ${account}.dkr.ecr.${region}.${domain}`);
    execSync(
      `aws ecr get-login-password --region ${region} ${profile ? `--profile ${profile}` : ''} | ${dockerCmd} login --username AWS --password-stdin ${account}.dkr.ecr.${region}.${domain}`,
