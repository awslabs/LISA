...
@api_wrapper
def list_jobs(event: dict, context: dict) -> dict[str, Any]:
    """List ingestion jobs for a repository.

    Args:
        event (dict): The Lambda event object containing:
            - pathParameters.repositoryId: The repository id to list jobs for
            - queryStringParameters.pageSize (optional): Number of jobs per page (default: 10)
            - queryStringParameters.timeLimit (optional): Time limit in hours (default: 720 - 30 days)
            - queryStringParameters.lastEvaluatedKey (optional): URL-encoded pagination key
        context (dict): The Lambda context object

    Returns:
        Dict containing jobs, pagination info, and metadata
    ...
    if "lastEvaluatedKey" in query_string_params:
        try:
            decoded_key = urllib.parse.unquote(query_string_params["lastEvaluatedKey"])
