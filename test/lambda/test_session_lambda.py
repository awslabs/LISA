...
import boto3
import pytest
from conftest import LambdaTestHelper, SessionTestHelper
...
def mock_session_functions():
    ...
    with patch("session.lambda_functions.table") as mock_table, patch(
        "session.lambda_functions.config_table"
    ) as mock_config_table, patch("session.lambda_functions.s3_resource") as mock_s3_resource, patch(
        "session.lambda_functions.s3_client"
    ) as mock_s3_client:
...
def test_get_session(sessions_table, mock_session_auth, lambda_context):
    ...
    with patch("session.lambda_functions.table", sessions_table), patch(
        "session.lambda_functions._update_session_with_current_model_config"
...
def test_put_session(sessions_table, config_table, mock_session_auth, lambda_context):
    ...
    # Create test event
    event = SessionTestHelper.create_session_event("test-session", "test-user")
    event["body"] = json.dumps({"messages": session_data["history"], "configuration": session_data["configuration"]})

    # Mock dependencies
    with patch("session.lambda_functions.table", sessions_table), patch(
        "session.lambda_functions.config_table", config_table
    ), patch("session.lambda_functions._is_session_encryption_enabled") as mock_encryption, patch(
        "session.lambda_functions._update_session_with_current_model_config"
    ) as mock_update_config:
...
def test_delete_session(sessions_table, mock_session_auth, lambda_context):
    ...
    with patch("session.lambda_functions.table", sessions_table), patch(
        "session.lambda_functions._delete_user_session"
    ) as mock_delete:
...
def test_delete_user_sessions(sessions_table, mock_session_auth, lambda_context):
    ...
    # Mock dependencies
    with patch("session.lambda_functions.table", sessions_table), patch(
        "session.lambda_functions._get_all_user_sessions"
    ) as mock_get_sessions, patch("session.lambda_functions._delete_user_session") as mock_delete:
...
def test_attach_image_to_session(mock_aws_services, lambda_context):
    ...
    with patch("session.lambda_functions.s3_client", mock_aws_services["s3"]), patch(
        "session.lambda_functions._generate_presigned_image_url"
    ) as mock_generate:
...
def test_get_current_model_config():
    ...
    mock_model_table.get_item.return_value = {
        "Item": {"model_id": "test-model", "model_config": {"features": ["feature1"], "streaming": True}}
    }
...
def test_update_session_with_current_model_config():
    ...
    current_model_config = {"features": ["new-feature"], "streaming": False, "modelType": "updated-type"}
...
def test_find_first_human_message():
    ...
    session = {
        ...
        "is_encrypted": False,
        "history": [{"type": "human", "content": "Hello world"}, {"type": "assistant", "content": "Hi there!"}],
    }
...
def test_generate_presigned_image_url(mock_aws_services):
    ...
    # Create a MagicMock for S3 client that returns a proper string
    mock_s3 = MagicMock()
    mock_s3.generate_presigned_url.return_value = "https://test-bucket.s3.amazonaws.com/test-key?signature=xyz"

    with patch("session.lambda_functions.s3_client", mock_s3):
...
def test_get_session_encrypted_decryption_error(sessions_table, mock_session_auth, lambda_context):
    ...
    with patch("session.lambda_functions.table", sessions_table), patch(
        "session.lambda_functions.decrypt_session_fields"
    ) as mock_decrypt:
...
def test_put_session_with_encryption_enabled(sessions_table, config_table, mock_session_auth, lambda_context):
    ...
    with patch("session.lambda_functions.table", sessions_table), patch(
        ...
        "session.lambda_functions.migrate_session_to_encrypted"
